...
def convert_catalog():
    ...
    summary_file = "/mnt/DATA2/YiyuanZhong/LP_catalog/Hawaii/events_1986to2011.txt"
    station_archive_file = (
        "/mnt/DATA2/YiyuanZhong/LP_catalog/Hawaii/events_1986to2011.hypo"
    )
...
def sac2mseed():
    ...
    rand_events_idxs = np.sort(
        np.random.default_rng(seed=50).choice(
            rg_ids, size=n_lp_events * 2, replace=False
        )
    )
    ...
    num_processes = 50
    logger.info(
        f"Starting converting sac to mseed. Save dir: {hawaii.save_dir}. Number of processes: {num_processes}."
    )
    hawaii.convert_sac_to_mseed(
    ...
    running_time = str(datetime.timedelta(seconds=t2 - t1))
    logger.info(
        f"Finished converting SAC files to MSEED files. Running time {running_time}"
    )
...
def plot_waveforms_gaps():
    waveform_table = pd.read_csv(
        hawaii.save_dir / "sac2mseed_log" / "abnormal_traces.csv"
    )
    hawaii.plot_sac_waveforms_with_phases_in_gap(
        num=60,
...
def plot_waveforms():
    waveform_table = pd.read_csv(hawaii.save_dir / "sac2mseed_log" / "convert.csv")
    ...
    hawaii.plot_waveforms(
        ...
        xrangemin=-2,
        xrangemax=2,
        data_dir=hawaii.save_dir / "sac2mseed",
        fig_dir=hawaii.save_dir / "sac2mseed_lp_fig",
    )
    ...
    hawaii.plot_waveforms(
        ...
        xrangemax=2,
        data_dir=hawaii.save_dir / "sac2mseed",
        fig_dir=hawaii.save_dir / "sac2mseed_rg_fig",
    )
...
def download():
    ...
    logger.info(
        f"Selecting the data after 2009-03-31T23:59:59.000000Z with both manuall P and S picks..."
    )
    metadata2009to2011 = catalog_table[
        (catalog_table["source_origin_time"] > "2009-03-31T23:59:59.000000Z")
        & (pd.notna(catalog_table["trace_s_arrival_time"]))
        & (pd.notna(catalog_table["trace_p_arrival_time"]))
    ].copy()
    lp_2009_2011 = metadata2009to2011[metadata2009to2011["source_type"] == "lp"].copy()
    rg_2009_2011 = metadata2009to2011[metadata2009to2011["source_type"] != "lp"].copy()

    lp_ids = lp_2009_2011.drop_duplicates(
        subset="source_id", keep="first", ignore_index=True, inplace=False
    )["source_id"]
    rg_ids = rg_2009_2011.drop_duplicates(
        subset="source_id", keep="first", ignore_index=True, inplace=False
    )["source_id"]
    ...
    rand_rg_table = rg_2009_2011[
        rg_2009_2011["source_id"].isin(rand_events_idxs)
    ].copy()

    new_table = pd.concat([lp_2009_2011, rand_rg_table], ignore_index=True).copy()
    ...
    hawaii.download_data(
        ...
        time_after=60,
        sampling_rate=100,
        download_dir=hawaii.save_dir / "downloaded_mseed",
        num_processes=num_processes,
    )
    t2 = time.perf_counter()
    running_time = str(datetime.timedelta(seconds=t2 - t1))
